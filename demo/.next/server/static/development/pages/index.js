...
(function (global, factory) {
  ...
  var lodash_1 = lodash.maxBy;
  var lodash_2 = lodash.minBy;
  var lodash_3 = lodash.uniq;
  var lodash_4 = lodash.values;
  ...
  function isValidTimestamp(_timestamp) {
    var newTimestamp = new Date(_timestamp).getTime();
    return isNumeric(newTimestamp);
  }

  function _validate(data, songLength, pitches, octaves, baseOctave) {
    var totalNotes = pitches.length * VALID_OCTAVES;
    var isRangeValid = octaves * pitches.length + baseOctave * pitches.length < totalNotes;
    var isDataValid = Array.isArray(data) && data.every(function (point) {
      return Array.isArray(point) && isValidTimestamp(point[0]) && isNumeric(point[1]);
    });

    if (!isDataValid) {
      throw new Error("Please format your data as a two-dimensional array of unix timestamps and numeric values, e.g. [[1586969694206, 2.3], [1596969695555, 5.3]]");
    }

    if (!isRangeValid) {
      throw new Error("Invalid Range. Please choose a lower base octave or a smaller octave range.");
    }
  ...
  var _C0$C0$Db0$D0$D0$Eb;

  var notes = (_C0$C0$Db0$D0$D0$Eb = {
    C0: 16.35,
    "C#0": 17.32,
    Db0: 17.32,
    D0: 18.35,
    ...
    C8: 4186.01,
    "C#8": 4434.92
  }, _defineProperty(_C0$C0$Db0$D0$D0$Eb, "Db7", 4434.92), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "D8", 4698.63), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "D#8", 4978.03), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "Eb8", 4978.03), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "E8", 5274.04), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "F8", 5587.65), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "F#8", 5919.91), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "Gb8", 5919.91), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "G8", 6271.93), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "G#8", 6644.88), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "Ab8", 6644.88), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "A8", 7040.0), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "A#8", 7458.62), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "Bb8", 7458.62), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "B8", 7902.13), _defineProperty(_C0$C0$Db0$D0$D0$Eb, "C9", 8372.019), _C0$C0$Db0$D0$D0$Eb);
  ...
  var Sonify = function Sonify(_ref) {
    var _ref$pitches = _ref.pitches,
        pitches = _ref$pitches === void 0 ? ["C", "C#", "D", "D#", "E", "F", "Gb", "G", "A", "Ab", "Bb", "B"] : _ref$pitches,
        _ref$octaveRange = _ref.octaveRange,
        octaveRange = _ref$octaveRange === void 0 ? 2 : _ref$octaveRange,
        _ref$baseOctave = _ref.baseOctave,
        baseOctave = _ref$baseOctave === void 0 ? 3 : _ref$baseOctave,
        _ref$glissando = _ref.glissando,
        glissando = _ref$glissando === void 0 ? false : _ref$glissando,
        _ref$staticRhythm = _ref.staticRhythm,
        staticRhythm = _ref$staticRhythm === void 0 ? false : _ref$staticRhythm,
        _ref$onEnded = _ref.onEnded,
        onEnded = _ref$onEnded === void 0 ? function () {} : _ref$onEnded,
        _ref$data = _ref.data,
        data = _ref$data === void 0 ? [] : _ref$data,
        songLength = _ref.songLength;

    _classCallCheck(this, Sonify);

    _validate(data, songLength, pitches, octaveRange, baseOctave);

    this.pitches = Object.keys(notes).filter(function (note) {
      return pitches.includes(note.replace(/\d/g, ""));
    }).map(function (pitchName) {
      return notes[pitchName];
    });
  ...
  function _transform(data) {
    ...
    var pitchedData = _mapNodesToPitches.call(this, data);

    var timedData = _mapTimeToNoteLength.call(this, pitchedData);
  ...
  function _mapNodesToPitches(data) {
    ...
    var minDataPoint = lodash.minBy(data, function (x) {
      return x[1];
    })[1];

    var maxDataPoint = lodash.maxBy(data, function (x) {
      return x[1];
    })[1];

    return data.map(function (point) {
      var factor = percent(point[1], minDataPoint, maxDataPoint - minDataPoint);
      return [point[0], Math.round(factor * (_this.maxPitch - _this.minPitch) + _this.minPitch)];
    });
  }
  /**
  ...
  function _mapTimeToNoteLength(data) {
    ...
    var earliestTS = data[0][0];
    var latestTS = data[data.length - 1][0];
    var domain = latestTS - earliestTS;
    var avgNoteLength = this.songLength / data.length;
    var noteLength;
    return data.map(function (point, i) {
      if (i !== data.length - 1) {
        if (_this2.staticRhythm) {
          // If the rhythm is static, use avgNoteLength for every note
          return [].concat(_toConsumableArray(point), [avgNoteLength]);
        } else {
          ...
          var currentPointInSong = percent(point[0], earliestTS, domain) * _this2.songLength;

          var nextPointInSong = percent(data[i + 1][0], earliestTS, domain) * _this2.songLength;

          noteLength = nextPointInSong - currentPointInSong;
          return [].concat(_toConsumableArray(point), [noteLength]);
        }
      } else {
        ...
        noteLength = _this2.staticRhythm ? avgNoteLength * 2 : avgNoteLength;
        return [].concat(_toConsumableArray(point), [noteLength]);
      }
    });
  }
  ...
  function _createSound(freq, nextFreq, noteLength) {
    ...
    if (this.glissando) {
      ...
      this.oscillator.frequency.exponentialRampToValueAtTime(nextFreq, this.currentTime + noteLength);
    } else {
      // If glissando is false, instead we want to add a slight space between each
      // note so we can differentiate the data points
      this.gainNode.gain.setTargetAtTime(0, this.currentTime + noteLength - noteLength / this.songLength, 0.015);
  ...
  Sonify.prototype.play = function () {
    ...
    this.isPlaying = true; // Create the audio context

    _setContext.call(this); // Create gain and oscillator nodes


    this.gainNode = Sonify.context.createGain();
    this.oscillator = Sonify.context.createOscillator(); // Connect the oscillator and gain to the context destination

    this.oscillator.connect(this.gainNode);
    this.gainNode.connect(Sonify.context.destination); // Start the oscillator node
    ...
    for (var i = 0; i < this.data.length; i++) {
      ...
      freq = this.pitches[this.data[i].pitch]; // Call create sound with frequencies and note length

      _createSound.call(this, freq, nextFreq, this.data[i].noteLength);
    }

    this.oscillator.stop(this.currentTime); // Remove the "click" by setting the gain to 0 before the end of the last note

    this.gainNode.gain.setTargetAtTime(0, this.currentTime - this.songLength / this.data.length, 0.015);

    this.oscillator.onended = function (e) {
...
/* harmony import */ var sonify__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! sonify */ "../build.js");
...
/* harmony import */ var _components_GithubCorner__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../components/GithubCorner */ "./components/GithubCorner.js");
...
/* harmony default export */ __webpack_exports__["default"] = (function () {
  return react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement("div", {
    ...
    __self: this
  }, "Sonify Demo")), react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(_components_GithubCorner__WEBPACK_IMPORTED_MODULE_2__["default"], {
    __source: {
      fileName: _jsxFileName,
      lineNumber: 19
    },
    __self: this
  }), react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(_components_Editor__WEBPACK_IMPORTED_MODULE_3__["default"], {
    __source: {
      fileName: _jsxFileName,
      lineNumber: 20
    },
